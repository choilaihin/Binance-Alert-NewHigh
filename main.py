...
def main():
    ...
    exclusion_usdt_symbols = ["BUSDUSDT", "USDCUSDT",
                              "TUSDUSDT", "SUSDUSDT", "PAXUSDT", "USDPUSDT"]
    ...
    def handle_socket_message(msg):
        ...
        if now > initialisation_1min and initialisation_phase:
            ...
            loggerdailyhigh.info(
                f"There are {len(ticker_list)} symbols, updating database...")
        ...
        if database_phase:
            try:
                ...
                loggerdailyhigh.info(
                    f"Unable to connect to database. Error: {e}")
            ...
            for symbol in ticker_list:
                ...
                try:
                    ...
                    cursor = conn.execute(
                        f"SELECT count(name) FROM sqlite_master WHERE type='table' AND name={symbol_string}")
                    ...
                    for candlestick in candlesticks:
                        if start_inserting:
                            ...
                            conn.execute(
                                f"INSERT INTO {symbol_string} (TIME,OPEN,HIGH,LOW,CLOSE,VOLUME) VALUES ({candlestick[0]},{float(candlestick[1])},{float(candlestick[2])},{float(candlestick[3])},{float(candlestick[4])},{float(candlestick[5])})")
                        else:
                            # check whether next candlestick exist in table
                            cursor = conn.execute(
                                f"SELECT rowid FROM {symbol_string} WHERE TIME = ?", (candlestick[0]+86400000,))
                            if cursor.fetchone() is None:
                                # insert/update current candlestick and insert rest if the next candlestick is not found
                                cursor = conn.execute(
                                    f"SELECT rowid FROM {symbol_string} WHERE TIME = ?", (candlestick[0],))
                                if cursor.fetchone() is None:
                                    ...
                                    conn.execute(f"UPDATE {symbol_string} set OPEN = ?, HIGH = ?, LOW = ?, CLOSE = ?, VOLUME = ? where TIME = {candlestick[0]}", (float(
                                        candlestick[1]), float(candlestick[2]), float(candlestick[3]), float(candlestick[4]), float(candlestick[5]),))
                                # insert the remaining candlesticks without checking for existence
            ...
            for symbol in ticker_list:
                ...
                try:
                    ...
                    cursor_60d = conn.execute(
                        f"SELECT MAX(HIGH) FROM {symbol_string} WHERE TIME > {lower_thres_time_60d} AND TIME < {upper_thres_time}")
                    cursor_alltime = conn.execute(
                ...
                for ticker in msg['data']:
                    ...
                    if symbol in ticker_high_dict and price and ticker_high_dict[symbol]['alltime'] and ticker_high_dict[symbol]['60d'] and ticker_high_dict[symbol]['30d']:
                        if price > ticker_high_dict[symbol]['alltime']:
                            ...
                            loggerdailyhigh.info(
                                f'{now:%Y-%m-%d %H:%M:%S} {symbol} reached all time high of {price}')
                        ...
                        if price > ticker_high_dict[symbol]['60d']:
                            ...
                            loggerdailyhigh.info(
                                f'{now:%Y-%m-%d %H:%M:%S} {symbol} reached 60 day high of {price}')
                            del ticker_high_dict[symbol]
                            continue
